...
initial_peak_rate_threshold = 0.03

initial_samples_per_period = 1000

contWav = alg.wavelet_generator(x,y)
allSegments, polynomialRegression = alg.full_segment_analysis(x,y, name=name, peak_rate_threshold=initial_peak_rate_threshold, join=True, segment_mode='poly', plot=False, title='short', samples_per_period=initial_samples_per_period, bands=(1,31), sampling_period=1)
...
y_segment, x_segment = alg.resample_to_peak(np.ones(len(x_segment)), x_segment, sample_rate_override=len(x))
...
freq_slider = Slider(axfreq, 'Peak Rate Threshold', 0.01, 0.1, valinit=initial_peak_rate_threshold)
...
def update(val):
    peak_rate_threshold = freq_slider.val
    samples_per_period = amp_slider.val
    allSegments, polynomialRegression = alg.full_segment_analysis(x,y, name=name, peak_rate_threshold=peak_rate_threshold, join=True, segment_mode='poly', plot=False, title='short', samples_per_period=samples_per_period, bands=(1,31), sampling_period=1)
    for idx, val in enumerate(polynomialRegression):
        coeffs = np.asarray(list(val[0].coef_)[::-1] + [val[0].intercept_])
